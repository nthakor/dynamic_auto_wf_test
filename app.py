...
with st.sidebar:
    ...
    # ── Waterfall date filters ─────────────────────────────────────────
    st.markdown("### 📅 Waterfall Window")
    ...
    # ── Bad Rate date filters ─────────────────────────────────────────
    sel_br_years = []
    sel_br_quarters = []
    use_bad_window = False

    if cfg.date_col and "_year" in df_enriched.columns:
        with st.expander("📉 Bad Rate Window  _(optional)_", expanded=False):
            st.caption(
                "Restrict **which rows count toward bad rates** independently of the "
                "waterfall volume window.  Useful when decisioning data and performance "
                "data have different observation periods."
            )
            use_bad_window = st.checkbox("Enable separate bad rate window", value=False)
            if use_bad_window:
                sel_br_years = st.multiselect(
                    "Bad Rate — Year",
                    all_years,
                    default=all_years,
                    format_func=str,
                ...
                sel_br_quarters = st.multiselect(
                    "Bad Rate — Quarter",
                    all_quarters,
                    default=all_quarters,
                    format_func=lambda q: f"Q{q}",
    ...
    # ── Apply all filters ─────────────────────────────────────────────
    # Waterfall pool (volume counts use this)
    df = filter_data(
        df_enriched,
    ...
    # Bad rate pool (bad counts use this, cat filters always apply)
    if use_bad_window:
        df_bad = filter_data(
            df_enriched,
            date_col=cfg.date_col,
            years=sel_br_years or None,
            quarters=sel_br_quarters or None,
            cat_filters=cat_filter_values or None,
        )
    else:
        df_bad = None  # signals analytics to use df for both
    st.markdown("---")
    ...
    bad_window_note = (
        f" · Bad rate window: **{len(df_bad):,}**" if df_bad is not None else ""
    )
    st.caption(
        f"**{len(df):,}** of {len(df_raw):,} records (waterfall){bad_window_note}  \n"
